package com.bravesoft.riumachi.wheelChoice;
...
class PickerUIAdapter extends ArrayAdapter<String> {
	 ...
	 * @param position
	 *            position to set in the center of the list. By default, is the
	 *            half of items.
	 * @param itemsClickables
	 *            to set if items can be clicked.
	 * @param isInEditMode
	 *            to avoid to set styles.
	 */
	public PickerUIAdapter(Context context, int resource, List<String> items,
			int position, boolean itemsClickables, boolean isInEditMode) {
		super(context, resource, items);
		this.mContext = context;
		this.itemsClickables = itemsClickables;
		this.isInEditMode = isInEditMode;
		positionsNoClickables = new SparseIntArray(items.size());
		setItems(items, position % items.size());
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String option = items.get(position % items.size());
		 ...
		 */
		if (!isInEditMode) {
			setTextItemStyle(textItem, position % items.size());
		}
	...
	/**
	 * This method sets the appropriate style to each of the components to get a
	 * carousel effect.
	 * 
	 * @param textItem
	 *            the TextView of the current position of the actual item
	 * @param position
	 *            the current position of the actual item
	 */
	private void setTextItemStyle(TextView textItem, int position) {

		if (position % items.size() == centerPosition) {
		...
		} else if (position % items.size() - 1 == centerPosition) {
			textItem.setTextAppearance(mContext,
					R.style.PickerUI_Near_Center_Item);
			setTextNoCenterColor(textItem);
			textItem.setRotationX(ROTATION_TWICE_ABOVE);
			textItem.setAlpha((float) 1.0);
		} else if (position % items.size() - 2 == centerPosition) {
			textItem.setTextAppearance(mContext,
					R.style.PickerUI_Far_Center_Item);
			setTextNoCenterColor(textItem);
			textItem.setRotationX(ROTATION_FIRST);
			textItem.setAlpha((float) 0.0);
		} else if (position % items.size() + 1 == centerPosition) {
			textItem.setTextAppearance(mContext,
					R.style.PickerUI_Near_Center_Item);
			setTextNoCenterColor(textItem);
	...
	/**
	 * This method is used to set the items to display in the panel and the
	 * empty rows int the beginning and in the end.
	 * 
	 * @param rawItems
	...
	void setItems(List<String> rawItems, int position) {
		...
		if (position % items.size() == -1) {
			...
			centerPosition = position % items.size() + 2;
		}
	...
	/**
	 * It saves in {@link PickerUIAdapter#centerPosition} and notify adapter.
	 * Then, the adapter in
	 * {@link PickerUIAdapter#setTextItemStyle(TextView, int)} set the
	 * appropriate style.
	 * 
	 * @param position
	 *            this is the position in the center of the list
	 */
	public void handleSelectEvent(int position) {
		this.centerPosition = position % items.size();
		this.notifyDataSetChanged();
	}
	 ...
	 * 
	 * @param itemsClickables
	 *            indicates whether the items are clickable or not.
	 */
	...
	/**
	 * The first two positions must be empty. The last 2 positions must be empty
	 * too.
	 */
	 ...
	 * 
	 * if it is allowed that elements can be clicked, the first two positions
	 * and the last 2 positions can not be pressed.
	 * 
	 * @param position
	 *            the current position of the actual item
	...
	public boolean isEnabled(int position) {
		if (!itemsClickables) {
			...
			boolean isClickable = positionsNoClickables.get(
					position % items.size(), -1) == -1;
			return isClickable && super.isEnabled(position % items.size());
		}
	}
